...
help:
	...
	@echo "docs                    			# generate API docs (Scribe)"
...
start: setup setup-network
	...
	docker-compose -f docker-compose.local.yml exec app bash -c 'test -f config/scribe.php || php artisan vendor:publish --tag=scribe-config --ansi --force'
...
docs:
	docker-compose -f docker-compose.local.yml exec app php artisan scribe:generate --force
